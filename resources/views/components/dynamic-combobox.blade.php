...
<script>
    ...
    return {
        ...
        selectItem(item) {
            this.search = item.name; // Display the selected state name
            this.selectedId = item.id; // Save the state ID
    ...
    if (defaultValue) {
        ...
        console.log('Fetching state data for ID:', defaultValue);

        // Fetch the state name by ID
        fetch(`${fetchUrl}?id=${defaultValue}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched Data:', data);
                if (data) {
                    this.search = data.name; // Set the state name
                    this.items = [data]; // Add to items array
                }
            })
            .catch(error => {
