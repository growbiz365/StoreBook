<x-app-layout>
    <x-breadcrumb :breadcrumbs="[['url' => '/', 'label' => 'Home'],['url' => '/general-items-dashboard', 'label' => 'General Items Dashboard'], ['url' => '/item-types', 'label' => 'Item Types'], ['url' => '#', 'label' => 'Add Item Type']]" />

    <x-dynamic-heading title="Add Item Type" />
    ...
    <form action="{{ route('item-types.store') }}" method="POST">
        ...
        <div class="bg-white shadow-lg sm:rounded-lg border border-gray-200 p-8">
            <div class="pb-10 mb-10 border-b border-gray-150 my-8">
                ...
                <p class="mt-1 text-sm text-gray-600">Please provide details of the item type.</p>
                ...
                <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                        <x-text-input name="item_type" value="{{ old('item_type') }}" required />
                        </div>
                    </div>
