<x-app-layout>
    @section('title', 'Add Arms Condition - Arms Management')
    <x-breadcrumb :breadcrumbs="[['url' => '/', 'label' => 'Home'],['url' => '/arms-dashboard', 'label' => 'Arms Management'], ['url' => '/arms-conditions', 'label' => 'Arms Conditions'], ['url' => '#', 'label' => 'Add Arms Condition']]" />

    <x-dynamic-heading title="Add Arms Condition" />

    <form action="{{ route('arms-conditions.store') }}" method="POST">
        @csrf
        <div class="bg-white shadow-lg sm:rounded-lg border border-gray-200 p-8">
            <div class="pb-10 mb-10 border-b border-gray-150 my-8">
                ...
                <p class="mt-1 text-sm text-gray-600">Please provide details of the arms condition.</p>
                ...
                <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                        <x-text-input name="arm_condition" value="{{ old('arm_condition') }}" placeholder="e.g., New, Used, Excellent, Good" required />
                        </div>
                    </div>
