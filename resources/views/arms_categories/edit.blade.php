<x-app-layout>
    @section('title', 'Edit Arms Category - Arms Management')
    <x-breadcrumb :breadcrumbs="[['url' => '/', 'label' => 'Home'],['url' => '/arms-dashboard', 'label' => 'Arms Management'], ['url' => '/arms-categories', 'label' => 'Arms Categories'], ['url' => '#', 'label' => 'Edit Arms Category']]" />

    <x-dynamic-heading title="Edit Arms Category" />

    <form action="{{ route('arms-categories.update', $armsCategory) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="bg-white shadow-lg sm:rounded-lg border border-gray-200 p-8">
            <div class="pb-10 mb-10 border-b border-gray-150 my-8">
                ...
                <p class="mt-1 text-sm text-gray-600">Please update the details of the arms category.</p>
                ...
                <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                        <x-text-input name="arm_category" value="{{ old('arm_category', $armsCategory->arm_category) }}" placeholder="e.g., Handgun, Rifle, Shotgun" required />
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-1 mb-4 ml-4">
                        ...
                        <div class="mt-2">
                            <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                <option value="1" {{ old('status', $armsCategory->status) === '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('status', $armsCategory->status) === '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                    </div>
                </div>
