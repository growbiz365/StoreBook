<x-app-layout>
    @section('title', 'Edit Expense - Expense Management - StoreBook')
    <x-breadcrumb :breadcrumbs="[
        ['url' => '/', 'label' => 'Home'],
        ['url' => '/expenses/dashboard', 'label' => 'Expense dashboard'],
        ['url' => route('expenses.index'), 'label' => 'Expenses'],
        ['url' => '#', 'label' => 'Edit Expense']
    ]" />

    <x-dynamic-heading title="Edit Expense" />

    <div class="bg-white shadow-lg sm:rounded-lg border border-gray-200 p-6">
        <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900 mb-4">Expense Information</h2>
            <p class="text-sm text-gray-600 mb-6">Update the expense record. This will also update the linked bank ledger and journal entries.</p>
        </div>

        <form action="{{ route('expenses.update', $expense) }}" method="POST" enctype="multipart/form-data" id="editExpenseForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <!-- Date -->
                <div>
                    <x-input-label for="date_added" value="Date" />
                    <input type="date" id="date_added" name="date_added" 
                        value="{{ old('date_added', $expense->date_added->format('Y-m-d')) }}"
                        required
                ...
                <!-- Expense Head -->
                <div>
                    <x-input-label for="chart_of_account_id" value="Expense Head" />
                    <select id="chart_of_account_id" name="chart_of_account_id" data-required="true"
                        ...
                        @foreach($expenseAccounts as $expenseAccount)
                            ...
                            <option value="{{ $expenseAccount['id'] }}"
                                {{ old('chart_of_account_id', $expense->expenseHead->chart_of_account_id ?? '') == $expenseAccount['id'] ? 'selected' : '' }}>
                                {{ $expenseAccount['name'] }}
                            </option>
                        @endforeach
                ...
                <!-- Bank -->
                <div>
                    <x-input-label for="bank_id" value="Bank Account" />
                ...
                <div>
                    ...
                    <input type="number" id="amount" name="amount" step="1" min="1"
                        value="{{ old('amount', round($expense->amount)) }}"
                        required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            ...
            <div class="mt-6">
                ...
                <textarea id="details" name="details" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                    placeholder="Optional expense details...">{{ old('details', $expense->details) }}</textarea>
            ...
            @if($expense->attachments->count() > 0)
                <div class="mt-6">
                    <div class="mb-4">
                        <h3 class="text-md font-semibold text-gray-900 mb-1">Current Attachments</h3>
                        <p class="text-xs text-gray-600">Existing attachments for this expense.</p>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                        @foreach($expense->attachments as $attachment)
                            <div class="bg-gray-50 rounded-md p-3 flex flex-col">
                                <div class="flex items-center mb-1">
                                    <svg class="h-4 w-4 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                    </svg>
                                    <span class="text-xs font-medium text-gray-900 truncate flex-1" title="{{ $attachment->original_name }}">
                                ...
                                <div class="flex items-center justify-between mt-auto">
                                    ...
                                    <button type="button" onclick="deleteAttachment({{ $attachment->id }})" class="text-xs text-red-600 hover:text-red-900">
                                        Delete
                                    </button>
                                </div>
                            </div>
            ...
            <div class="mt-6">
                <div class="mb-4">
                    <h3 class="text-md font-semibold text-gray-900 mb-1">Add New Attachments</h3>
                    <p class="text-xs text-gray-600">Upload additional documents (receipts, invoices, etc.)</p>
                </div>
            ...
            <div class="flex items-center gap-4 pt-4 mt-6 border-t">
                ...
                <a href="{{ route('expenses.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300">
                    Cancel
                </a>
            </div>
        </form>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize bank balance for pre-selected bank (from existing expense data)
            if (bankSelect.value) {
