<x-app-layout>
    @section('title', 'Bank Transfer Details - Bank Management - StoreBook')
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-3">
            <a href="{{ route('bank-transfers.edit', $bankTransfer) }}" 
            ...
            <form action="{{ route('bank-transfers.destroy', $bankTransfer) }}" method="POST" 
                onsubmit="return confirm('Are you sure you want to delete this bank transfer? This will reverse all ledger entries and cannot be undone.');">
                @csrf
                @method('DELETE')
                <button type="submit" 
    ...
    <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div class="p-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">@businessDate($bankTransfer->transfer_date)</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Amount</label>
                        <p class="mt-1 text-sm font-semibold text-gray-900">{{ number_format(round($bankTransfer->amount), 0) }}</p>
                    </div>
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-600">Details</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $bankTransfer->details ?: 'No details provided' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-4">
                ...
                <div class="space-y-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-600">From Account</label>
                        <p class="mt-1 text-sm font-medium text-gray-900">
                            {{ $bankTransfer->fromAccount->chartOfAccount->name ?? $bankTransfer->fromAccount->account_name }}
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">To Account</label>
                        <p class="mt-1 text-sm font-medium text-gray-900">
                            {{ $bankTransfer->toAccount->chartOfAccount->name ?? $bankTransfer->toAccount->account_name }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg lg:col-span-2" x-data="{ journalEntriesOpen: false }">
            <div class="p-4">
                ...
                <div x-show="journalEntriesOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-2">
                    @php
                        $journalEntries = \App\Models\JournalEntry::where('voucher_id', $bankTransfer->id)
                    ...
                    @else
                        <p class="text-sm text-gray-500">No journal entries found for this transfer.</p>
                    @endif
                </div>
            </div>
        </div>
                ...
                <div x-show="additionalInfoOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-2">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $bankTransfer->user->name }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Created At</label>
                            <p class="mt-1 text-gray-900">@businessDateTime($bankTransfer->created_at)</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Last Updated</label>
                            <p class="mt-1 text-gray-900">@businessDateTime($bankTransfer->updated_at)</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        function deleteAttachment(id) {
            if (confirm('Are you sure you want to delete this attachment?')) {
                fetch(`/bank-transfers/attachments/${id}`, {
                    method: 'DELETE',
                    headers: {
