<x-app-layout>
    @section('title', 'Edit Approval #' . $approval->id . ' - StoreBook')
    ...
    <x-dynamic-heading 
        :title="'Edit Approval ' . $approval->approval_number" 
        :subtitle="'Modify approval details and items'"
        :icon="'fas fa-edit'"
    />
    ...
    <form method="POST" action="{{ route('approvals.update', $approval) }}" id="approvalForm">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="searchable-select-container relative">
                            <input type="text" 
                                   ...
                                   autocomplete="off"
                                   value="{{ old('party_search_input', $approval->party->name . ($approval->party->cnic ? ' (CNIC: ' . $approval->party->cnic . ')' : '')) }}">
                            <input type="hidden" name="party_id" id="party_id" class="selected-item-id" value="{{ old('party_id', $approval->party_id) }}">
                    ...
                    <div>
                        ...
                        <input type="date" name="approval_date" id="approval_date" 
                            value="{{ old('approval_date', $approval->approval_date->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="notes" id="notes" 
                            value="{{ old('notes', $approval->notes) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                ...
                <div class="border-t border-gray-200 pt-4">
                    ...
                    <div class="overflow-visible border border-gray-200 rounded-md">
                            ...
                            <tbody id="general_items_container" class="bg-white divide-y divide-gray-200">
                                @foreach($approval->generalItems as $item)
                                    <tr class="general-item-row hover:bg-gray-50">
                                        <td class="px-3 py-2">
                                            <div class="searchable-select-container relative">
                                                <input type="text" 
                                                       ...
                                                       autocomplete="off"
                                                       value="{{ old('general_lines.' . $loop->index . '.general_item_id', $item->generalItem->item_name ?? '') }}"
                                                       data-index="{{ $loop->index }}">
                                                <input type="hidden" name="general_lines[{{ $loop->index }}][general_item_id]" class="selected-item-id" value="{{ old('general_lines.' . $loop->index . '.general_item_id', $item->general_item_id) }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="number" name="general_lines[{{ $loop->index }}][qty]" required step="1" min="1" 
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="number" name="general_lines[{{ $loop->index }}][sale_price]" required step="1" min="0" 
                                        ...
                                        <td class="px-3 py-2">
                                            <span class="text-sm font-medium text-gray-900 general-line-total">{{ number_format($item->line_total, 2) }}</span>
                                        </td>
                        ...
                        <div id="general_items_empty" class="p-6 text-center text-gray-400 text-sm bg-gray-50" style="display: {{ $approval->generalItems->count() > 0 ? 'none' : 'block' }};">
                ...
                <div class="border-t border-gray-200 pt-4" style="display: none;">
                    ...
                    <div class="overflow-visible border border-gray-200 rounded-md">
                            ...
                            <tbody id="arms_container" class="bg-white divide-y divide-gray-200">
                                @foreach($approval->arms as $approvalArm)
                                    <tr class="arm-row hover:bg-gray-50">
                                        <td class="px-3 py-2">
                                            <div class="searchable-select-container relative">
                                                <input type="text" 
                                                       ...
                                                       autocomplete="off"
                                                       value="{{ old('arm_lines.' . $loop->index . '.arm_id', ($approvalArm->arm->arm_title ?? $approvalArm->arm->serial_no ?? '')) }}"
                                                       data-index="{{ $loop->index }}">
                                                <input type="hidden" name="arm_lines[{{ $loop->index }}][arm_id]" class="selected-arm-id" value="{{ old('arm_lines.' . $loop->index . '.arm_id', $approvalArm->arm_id) }}">
                                        ...
                                        <td class="px-3 py-2">
                                            <input type="number" name="arm_lines[{{ $loop->index }}][sale_price]" required step="1" min="0" 
                        ...
                        <div id="arms_empty" class="p-6 text-center text-gray-400 text-sm bg-gray-50" style="display: {{ $approval->arms->count() > 0 ? 'none' : 'block' }};">
                    ...
                    <div class="flex justify-between items-center bg-gray-50 rounded-md px-4 py-3">
                        <div class="flex items-center space-x-6">
                            <div>
                                ...
                                <span class="ml-2 text-sm font-semibold text-gray-900" id="total_items">{{ $approval->generalItems->count() }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-gray-500">Arms:</span>
                                <span class="ml-2 text-sm font-semibold text-gray-900" id="total_arms">{{ $approval->arms->count() }}</span>
                            </div>
                            <div class="border-l border-gray-300 pl-6">
                                <span class="text-xs text-gray-500">Total Value:</span>
                                <span class="ml-2 text-base font-bold text-teal-600" id="total_value">{{ number_format($approval->total_approved_value ?? 0, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <template id="general_item_template">
        <tr class="general-item-row hover:bg-gray-50">
            ...
            <td class="px-3 py-2">
                <input type="number" name="general_lines[INDEX][sale_price]" required step="1" min="0" 
                    class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-purple-500 focus:border-purple-500 general-sale-price">
            </td>
            <td class="px-3 py-2">
    ...
    <template id="arm_template">
        <tr class="arm-row hover:bg-gray-50">
            ...
            <td class="px-3 py-2">
                <input type="number" name="arm_lines[INDEX][sale_price]" required step="1" min="0" 
                    class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-blue-500 focus:border-blue-500 arm-sale-price">
            </td>
            <td class="px-3 py-2 text-center">
                <button type="button" class="remove-arm text-red-500 hover:text-red-700">
    ...
    <script>
        let generalItemIndex = {{ $approval->generalItems->count() }};
        let armIndex = {{ $approval->arms->count() }};
        ...
        class GeneralItemSearchableDropdown {
            ...
            selectItem(item) {
                ...
                // Add to global selected IDs (but exclude current item from exclusion list)
                updateSelectedIds();
                ...
                const existingInfo = this.container.querySelector('.item-info');
                if (existingInfo) {
                    existingInfo.remove();
                }
        ...
        function updateSummary() {
            ...
            document.querySelectorAll('.general-line-total').forEach(span => {
                totalValue += parseFloat(span.textContent.replace(/,/g, '')) || 0;
            });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('#general_items_container .general-item-row .searchable-select-container').forEach(container => {
                const row = container.closest('.general-item-row');
                const qtyInput = row.querySelector('.general-qty');
            ...
            document.querySelectorAll('#arms_container .arm-row .searchable-select-container').forEach(container => {
                const row = container.closest('.arm-row');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (armPricesInForm.length > armLinesInForm.length) {
                    ...
                    manualFormData.append('_method', method);
                    ...
                    .then(data => {
                        if (data && data.redirect) {
                            ...
                            window.location.href = '{{ route("approvals.index") }}?success=Approval updated successfully.';
                        }
                    })
                    .catch(error => {
