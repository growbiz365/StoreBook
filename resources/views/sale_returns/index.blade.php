<x-app-layout>
    @section('title', 'Sale Returns List - Sales Management - StoreBook')
    <x-breadcrumb :breadcrumbs="[['url' => '/', 'label' => 'Home'],['url' => '/sale-returns', 'label' => 'Sale Returns'],['url' => '#', 'label' => 'Returns']]" />
    ...
    <div class="bg-gradient-to-r from-orange-50 via-white to-white rounded-xl shadow-sm border border-orange-100 p-6 mb-6 mt-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Sale Returns</h1>
                    <p class="text-sm text-gray-500 mt-1">Manage and track all return transactions</p>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
            @can('create sale returns')    
            <a href="{{ route('sale-returns.create') }}" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium rounded-md shadow-sm transition-colors duration-150 ease-in-out">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Total Records: {{ $saleReturns->total() }}</p>
                    </div>
                </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($saleReturns as $saleReturn)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $saleReturn->party->name ?? '-' }}</div>
                                    <div class="text-sm text-gray-500">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800">
                                            {{ ucfirst($saleReturn->return_type) }}
                                        </span>
                                        @if($saleReturn->bank)
                                            • {{ $saleReturn->bank->chartOfAccount->name ?? $saleReturn->bank->account_name }}
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">@businessDate($saleReturn->return_date)</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">@currency($saleReturn->total_amount)</div>
                                    <div class="text-sm text-gray-500">
                                        Subtotal: @currency($saleReturn->subtotal)
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                        @if($saleReturn->status == 'draft') bg-gray-100 text-gray-800
                                        @elseif($saleReturn->status == 'posted') bg-orange-100 text-orange-800
                                        @else bg-red-100 text-red-800 @endif">
                                        {{ ucfirst($saleReturn->status) }}
                                    </span>
                                    @if($saleReturn->status == 'posted')
                                        <div class="text-xs text-gray-500 mt-1">
                                            {{ $saleReturn->generalLines->count() }} items • {{ $saleReturn->armLines->count() }} arms
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <a href="{{ route('sale-returns.show', $saleReturn) }}" 
                                        ...
                                        @endcan
                                        @can('edit sale returns')
                                        @if($saleReturn->canBeEdited())
                                            <a href="{{ route('sale-returns.edit', $saleReturn) }}" 
                                               class="text-orange-600 hover:text-orange-900" title="Edit Sale Return">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @if($saleReturn->canBeCancelled())
                                            <form action="{{ route('sale-returns.cancel', $saleReturn) }}" method="POST" class="inline">
                                                @csrf
        ...
        @else
            <div class="text-center py-12">
                ...
                <div class="mt-6">
                    <a href="{{ route('sale-returns.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500">
