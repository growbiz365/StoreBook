<x-app-layout>
    @section('title', 'Create Sale Return - Sales Management - StoreBook')
    <x-breadcrumb :breadcrumbs="[
        ['url' => '/', 'label' => 'Home'],
        ['url' => '/sale-returns', 'label' => 'Sale Returns'],
        ['url' => '#', 'label' => 'Create Sale Return']
    ]" />

    <x-dynamic-heading 
        :title="'Create Sale Return'" 
        :subtitle="'Add new sale return with general items and arms'"
        :icon="'fas fa-undo'"
    />
    <style>
    ...
    <form method="POST" action="{{ route('sale-returns.store') }}" id="saleReturnForm">
        ...
        <div class="bg-white shadow-lg rounded-lg border border-gray-200 p-6">
            ...
            <div id="customer_details_field" style="display: none;" class="bg-white border border-gray-200 rounded-lg p-3 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div class="md:col-span-2 lg:col-span-4">
                        ...
                        <textarea name="address" id="address" rows="2" 
                            class="w-full px-2 py-1 border border-gray-300 rounded text-xs focus:border-gray-400 focus:outline-none @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
                        @error('address')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div id="party_details_field" style="display: none;" class="bg-white border border-gray-200 rounded-lg p-3 mb-6">
                <h2 class="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                    <svg class="w-3 h-3 mr-1 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                    </svg>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                        ...
                        <textarea id="party_address" rows="2" readonly 
                                  class="w-full px-2 py-1 border border-gray-300 rounded text-xs bg-gray-100 text-gray-600"></textarea>
                    </div>
...
(function() {
    ...
    const isCreatePage = window.location.href.includes('/sale-returns/create');
    ...
    const isFromDifferentPage = document.referrer && 
        !document.referrer.includes('/sale-returns/create') && 
        !document.referrer.includes('sale-returns/create');
...
class ArmSearchableDropdown {
    ...
    // ===== DATA PERSISTENCE FOR SALE RETURN FORM =====

    // Function to save sale return form data to localStorage
    function saveSaleReturnFormData() {
        try {
            const formData = {
                business_id: '{{ session("active_business") }}', // Store business_id for validation
                form_type: 'sale_return', // Identify form type to prevent cross-form restoration
            ...
            generalRows.forEach((row, index) => {
                ...
                const salePrice = row.querySelector('.general-sale-price')?.value || '';

                if (generalItemId || qty || salePrice) {
            ...
            if (formData.return_type || formData.party_id || formData.bank_id || 
                ...
                localStorage.setItem('sale_return_form_data', JSON.stringify(formData));
                console.log('Sale return form data saved:', formData);
            }
        } catch (error) {
            console.error('Error saving sale return form data:', error);
        }
    }

    // Function to load saved sale return form data from localStorage
    function loadSavedSaleReturnData() {
        try {
            const savedData = localStorage.getItem('sale_return_form_data');
            if (!savedData) {
                console.log('No saved sale return data found');
                return;
            }

            const formData = JSON.parse(savedData);
            console.log('Loading saved sale return data:', formData);
            ...
            if (formData.return_type) {
                ...
                // Trigger return type change to show/hide fields
            ...
            if (formData.name_of_customer) {
                document.getElementById('name_of_customer').value = formData.name_of_customer;
            }
            ...
            if (formData.cnic) {
                document.getElementById('cnic').value = formData.cnic;
            }
            if (formData.licence_no) {
                document.getElementById('licence_no').value = formData.licence_no;
            }
            if (formData.licence_issue_date) {
                document.getElementById('licence_issue_date').value = formData.licence_issue_date;
            }
            if (formData.licence_valid_upto) {
                document.getElementById('licence_valid_upto').value = formData.licence_valid_upto;
            }
            if (formData.licence_issued_by) {
                document.getElementById('licence_issued_by').value = formData.licence_issued_by;
            }
            if (formData.re_reg_no) {
                document.getElementById('re_reg_no').value = formData.re_reg_no;
            }
            if (formData.dc) {
                document.getElementById('dc').value = formData.dc;
            }
            if (formData.Date) {
            ...
            console.log('Sale return form data restored successfully');
        } catch (error) {
            console.error('Error loading saved sale return form data:', error);
        }
    }

    // Function to clear saved sale return form data
    function clearSaleReturnFormData() {
        localStorage.removeItem('sale_return_form_data');
        localStorage.removeItem('sale_return_form_failed_submission');
        sessionStorage.removeItem('sale_return_form_submitting');
        console.log('Sale return form data cleared');
    }
    ...
    function addSaleReturnDataPersistenceListeners() {
        ...
        formFields.forEach(fieldId => {
            ...
            if (field) {
                field.addEventListener('input', saveSaleReturnFormData);
                field.addEventListener('change', saveSaleReturnFormData);
            }
        });
        ...
        const partySearchInput = document.getElementById('party_search_input');
        if (partySearchInput) {
            partySearchInput.addEventListener('input', saveSaleReturnFormData);
            partySearchInput.addEventListener('change', saveSaleReturnFormData);
        }
      ...
      document.addEventListener('DOMContentLoaded', function() {
          ...
          addSaleReturnDataPersistenceListeners();
        
        // Set up form submission tracking
        document.getElementById('saleReturnForm').addEventListener('submit', function(e) {
            sessionStorage.setItem('sale_return_form_submitting', 'true');
            localStorage.setItem('sale_return_form_failed_submission', 'true');
        });
        ...
        window.addEventListener('beforeunload', function(e) {
            ...
            if (!window.location.href.includes('/sale-returns/create')) {
                console.log('Leaving sale return create page - clearing flags');
                sessionStorage.removeItem('sale_return_form_submitting');
                localStorage.removeItem('sale_return_form_failed_submission');
            }
        });
        ...
        window.addEventListener('pageshow', function(event) {
            ...
            console.log('Sale return pageshow event:', {
                wasSubmitting: wasSubmitting,
            ...
            if (wasSubmitting && successMessage && isCreatePage) {
                console.log('Clearing sale return form data due to successful submission');
                clearSaleReturnFormData();
        ...
        setTimeout(() => {
            ...
            const successMessage = document.querySelector('.bg-green-100, .bg-green-50, [class*="success"], .alert-success');
            const errorMessages = document.querySelector('.bg-red-100, .text-red-600, .alert-danger, [class*="error"]');
            const hasSavedData = localStorage.getItem('sale_return_form_data');
            const hasFailedSubmissionFlag = localStorage.getItem('sale_return_form_failed_submission');
            
            console.log('Sale return restoration check:', {
                wasSubmitting: wasSubmitting,
                hasSuccessMessage: !!successMessage,
                hasErrorMessages: !!errorMessages,
            ...
            if ((wasSubmitting && !successMessage && errorMessages) || 
                ...
                console.log('Restoring sale return data after failed submission');
                setTimeout(() => {
                    loadSavedSaleReturnData();
                ...
                console.log('Fresh page load - not restoring sale return data');
            } else {
                console.log('Successful submission - not restoring sale return data');
            }
        }, 1500);
    ...
    setTimeout(() => {
        ...
        console.log('Sale return fallback check after 3 seconds:', {
            wasSubmitting: wasSubmitting,
            hasSuccessMessage: !!successMessage,
            hasErrorMessages: !!errorMessages,
            successMessageText: successMessage ? successMessage.textContent : 'none'
        ...
        if (successMessage || (!errorMessages && wasSubmitting)) {
            console.log('Fallback: Clearing sale return data due to success or no errors after submission');
            clearSaleReturnFormData();
        }
    }, 3000);
    ...
    setTimeout(() => {
        ...
        console.log('Sale return aggressive fallback check after 5 seconds:', {
            wasSubmitting: wasSubmitting,
            hasFailedSubmissionFlag: hasFailedSubmissionFlag,
            hasErrorMessages: !!errorMessages,
            hasSuccessMessage: !!successMessage,
        ...
        if (successMessage || (isCreatePage && !errorMessages)) {
            console.log('Aggressive fallback: Clearing sale return data - success or no errors on create page');
            clearSaleReturnFormData();
        } else {
            console.log('Aggressive fallback: Preserving sale return data - errors detected');
        }
    }, 5000);
    });

    // Debug functions (can be removed in production)
    window.debugSaleReturnForm = {
        save: saveSaleReturnFormData,
        load: loadSavedSaleReturnData,
        clear: clearSaleReturnFormData,
        show: () => {
            const data = localStorage.getItem('sale_return_form_data');
            console.log('Current saved sale return data:', data ? JSON.parse(data) : 'No data');
