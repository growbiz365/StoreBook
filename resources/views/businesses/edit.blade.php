<x-app-layout>
    @section('title', 'Edit Business - Settings - StoreBook')
    <x-breadcrumb :breadcrumbs="[['url' => '/', 'label' => 'Home'], ['url' => '/settings', 'label' => 'Settings'], ['url' => route('businesses.index'), 'label' => 'Businesses'], ['url' => '#', 'label' => 'Edit Business']]" />
    <x-dynamic-heading title="Edit Business" />
            ...
            <div class="pb-10 mb-10 border-b border-gray-150 my-8">
                ...
                <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                            <x-text-input name="business_name" value="{{ old('business_name', $business->business_name) }}" required />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="owner_name">Owner Name <span class="text-red-500">*</span></x-input-label>
                        <div class="mt-2">
                            <x-text-input name="owner_name" value="{{ old('owner_name', $business->owner_name) }}" required />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="cnic">CNIC</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="cnic" value="{{ old('cnic', $business->cnic) }}" data-mask="00000-0000000-0" placeholder="12345-1234567-1" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="contact_no">Contact No</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="contact_no" value="{{ old('contact_no', $business->contact_no) }}" data-mask="0000-0000000" placeholder="03XX-XXXXXXX" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="email">Email</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="email" type="email" value="{{ old('email', $business->email) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="address">Address</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="address" value="{{ old('address', $business->address) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <select name="country_id" id="country_id" class="block w-full border-gray-300 rounded-md shadow-sm chosen-select @error('country_id') border-red-500 @enderror">
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" @selected(old('country_id', $business->country_id)==$country->id)>{{ $country->country_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-1 mb-4 ml-4">
                        ...
                        <select name="timezone_id" id="timezone_id" class="block w-full border-gray-300 rounded-md shadow-sm chosen-select @error('timezone_id') border-red-500 @enderror">
                            ...
                            @foreach($timezones as $tz)
                                <option value="{{ $tz->id }}" @selected(old('timezone_id', $business->timezone_id)==$tz->id)>{{ $tz->timezone_name }}</option>
                            @endforeach
                        </select>
                        <div class="chosen-error-container">
                            @error('timezone_id')
                    ...
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <select name="currency_id" id="currency_id" class="block w-full border-gray-300 rounded-md shadow-sm chosen-select @error('currency_id') border-red-500 @enderror">
                            ...
                            @foreach($currencies as $cur)
                                <option value="{{ $cur->id }}" @selected(old('currency_id', $business->currency_id)==$cur->id)>{{ $cur->currency_name }}</option>
                            @endforeach
                        </select>
                        <div class="chosen-error-container">
                            @error('currency_id')
                        ...
                        <div class="mt-2">
                            <select name="package_id" id="package_id" class="block w-full border-gray-300 rounded-md shadow-sm @error('package_id') border-red-500 @enderror">
                                ...
                                @foreach($packages as $pkg)
                                    <option value="{{ $pkg->id }}" @selected(old('package_id', $business->package_id)==$pkg->id)>{{ $pkg->package_name }}</option>
                                @endforeach
                            </select>
                            @error('package_id')
                                <span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>
                    ...
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                            <select name="date_format" id="date_format" class="block w-full border-gray-300 rounded-md shadow-sm @error('date_format') border-red-500 @enderror">
                                ...
                                <option value="Y-m-d" @selected(old('date_format', $business->date_format)=='Y-m-d')>YYYY-MM-DD (2024-07-13)</option>
                                <option value="d/m/Y" @selected(old('date_format', $business->date_format)=='d/m/Y')>DD/MM/YYYY (13/07/2024)</option>
                                <option value="m/d/Y" @selected(old('date_format', $business->date_format)=='m/d/Y')>MM/DD/YYYY (07/13/2024)</option>
                                <option value="d-m-Y" @selected(old('date_format', $business->date_format)=='d-m-Y')>DD-MM-YYYY (13-07-2024)</option>
                            </select>
                            @error('date_format')
                                <span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>
                        ...
                        <div class="mt-2">
                            @if($business->logo)
                                <div class="mb-2 flex items-center gap-3">
                                    <img src="{{ asset('storage/' . $business->logo) }}" alt="Business logo" class="h-12 w-12 rounded-md object-contain border border-gray-200 bg-white">
            ...
            <div class="pb-10 mb-10 border-b border-gray-150 my-8">
                ...
                <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                            <x-text-input name="store_name" value="{{ old('store_name', $business->store_name) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="store_license_number">Business License Number</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="store_license_number" value="{{ old('store_license_number', $business->store_license_number) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="license_expiry_date">Business License Expiry Date</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="license_expiry_date" type="date" value="{{ old('license_expiry_date', $business->license_expiry_date) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="issuing_authority">Issuing Authority</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="issuing_authority" value="{{ old('issuing_authority', $business->issuing_authority) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="store_type">Business Type</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="store_type" value="{{ old('store_type', $business->store_type) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="ntn">NTN</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="ntn" value="{{ old('ntn', $business->ntn) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="strn">STRN</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="strn" value="{{ old('strn', $business->strn) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="store_phone">Business Phone</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="store_phone" value="{{ old('store_phone', $business->store_phone) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="store_email">Business Email</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="store_email" type="email" value="{{ old('store_email', $business->store_email) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="store_address">Business Address</x-input-label>
                        <div class="mt-2">
                            <x-text-input name="store_address" value="{{ old('store_address', $business->store_address) }}" />
                        </div>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        <x-input-label for="store_city_id">City</x-input-label>
                        <select name="store_city_id" id="store_city_id" class="block w-full border-gray-300 rounded-md shadow-sm chosen-select">
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" @selected(old('store_city_id', $business->store_city_id)==$city->id)>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-1 mb-4 ml-4">
                        <x-input-label for="store_country_id">Country</x-input-label>
                        <select name="store_country_id" id="store_country_id" class="block w-full border-gray-300 rounded-md shadow-sm chosen-select">
                            <option value="">Select Country</option>
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" @selected(old('store_country_id', $business->store_country_id)==$country->id)>{{ $country->country_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-1 mb-4">
                        ...
                        <div class="mt-2">
                            <x-text-input name="store_postal_code" value="{{ old('store_postal_code', $business->store_postal_code) }}" />
                        </div>
                    </div>
                </div>
            </div>
