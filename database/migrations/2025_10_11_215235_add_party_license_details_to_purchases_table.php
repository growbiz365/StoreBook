...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('purchases', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('purchases', 'party_license_no')) {
                $table->string('party_license_no')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_license_issue_date')) {
                $table->date('party_license_issue_date')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_license_valid_upto')) {
                $table->date('party_license_valid_upto')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_license_issued_by')) {
                $table->string('party_license_issued_by')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_re_reg_no')) {
                $table->string('party_re_reg_no')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_dc')) {
                $table->string('party_dc')->nullable();
            }
            if (!Schema::hasColumn('purchases', 'party_dc_date')) {
